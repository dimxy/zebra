            ...
            let mempool_req = mempool
                ...
                .map(|responder| {
                    responder.respond(mempool::Response::Transactions(vec![UnminedTx {
                        ...
                        size: 0,
                        // conventional_fee: Amount::zero(), // no such field in komodo
                    }]));
                });
...
async fn rpc_getaddresstxids_invalid_arguments() {
    ...
    assert_eq!(
        error.message,
        format!(
            "invalid address \"{}\": parse error: t-addr/r-addr decoding error",
            address.clone()
        )
    );

    // create a valid address
    let address = "RJXkCF7mn2DRpUZ77XBNTKCe55M2rJbTcu".to_string();
    let addresses = vec![address.clone()];
...
async fn rpc_getaddressutxos_invalid_arguments() {
    ...
    assert_eq!(
        error.message,
        format!("invalid address \"{address}\": parse error: t-addr/r-addr decoding error")
    );
...
#[ignore = "fix for komodo upgrades"] // fix for komodo upgrades
#[cfg(feature = "getblocktemplate-rpcs")]
#[tokio::test(flavor = "multi_thread")]
...
#[ignore = "private transactions are not supported in komodo"]
#[cfg(feature = "getblocktemplate-rpcs")]
#[tokio::test(flavor = "multi_thread")]
