...
lazy_static! {
    ...
    static ref NNDATA_MAIN: NNDataMain<'static> = NNDataMain::new();   // TODO: do we need a mutex here for the readonly object?
    /// static ref to parsed NN testnet pubkeys
    static ref NNDATA_TEST: NNDataTest<'static> = NNDataTest::new();
}

/// minimum number of signers for notarisation transactions
pub fn komodo_minratify (network: Network, height: &Height) -> i32 { 
    match network {
        Network::Mainnet => if *height < Height(90000) { 7 } else { 11 }, 
        Network::Testnet => 7,
    }
}
...
pub enum NotaryDataError {
    ...
    #[error("this height {0:?} must be validated by checkpoints")]
    CheckpointValidate(Height),
}

/// Notarisation constants: HF activation timestamps and heights, notary pubkeys for mainnet
...
struct NNDataMain<'a> {
    ...
    nS7Timestamp: DateTime32,
    nS7HardforkHeight: Height,

    nn_season: NNSeason<'a>,
}
...
impl<'a> NNDataMain<'a> {
    ...
    const NUM_KMD_SEASONS: usize = 8;

    /// kmd mainnet height since elected NNs active
    pub const KOMODO_NOTARIES_HARDCODED_HT: u32 = 180_000; 

    /// kmd mainnet height max height for hardcoded NNs
    const KOMODO_NOTARIES_HARDCODED_MAX_HT: u32 = 250_000; 
    ...
    {    
        ...
        let nS7Timestamp = DateTime32::from(1688132253);   // dPoW Season 7, Fri Jun 30 2023 13:37:33 GMT+0000
        let nS7HardforkHeight = Height(3484958);  // dPoW Season 7, Fri Jun 30 2023

        let KMD_SEASON_TIMESTAMPS = vec![DateTime32::from(1525132800), DateTime32::from(1563148800), nStakedDecemberHardforkTimestamp, nS4Timestamp, nS5Timestamp, nS6Timestamp, nS7Timestamp, DateTime32::from(1951328000)];
        // Note: a season height is the right end of the previous season 
        // and season height + 1 is the left begin of the current season:
        let KMD_SEASON_HEIGHTS = vec![Height(814000), Height(1444000), nDecemberHardforkHeight, nS4HardforkHeight, nS5HardforkHeight, nS6HardforkHeight, nS7HardforkHeight, Height(8113400)];

        // convert to vector for convenience
        let elected_source = NNDataMain::NOTARIES_ELECTED_SOURCE.into_iter().
            map(|v| v.into_iter().collect::<Vec<_>>()).
            collect::<Vec<Vec<_>>>();

        NNDataMain {
            nStakedDecemberHardforkTimestamp, nDecemberHardforkHeight, 
            nS4Timestamp, nS4HardforkHeight,
            nS5Timestamp, nS5HardforkHeight,
            nS6Timestamp, nS6HardforkHeight,
            nS7Timestamp, nS7HardforkHeight,
            nn_season: NNSeason::new(
                NNDataMain::NUM_KMD_NOTARIES,
                NNDataMain::NUM_KMD_SEASONS,
                KMD_SEASON_TIMESTAMPS, 
                KMD_SEASON_HEIGHTS, 
                &elected_source
            ),
        }
    ...
    /// a wrapper to check mainnet heights
    pub fn get_kmd_season(&self, height: &Height) -> Result<&'a NotarySeasonPubkeys, NotaryDataError>
    {
        if *height >= Height(NNDataMain::KOMODO_NOTARIES_HARDCODED_HT) {
            match self.nn_season.get_kmd_season_internal(height) {
                Some(r) => { return Ok(r); },
                None => {       
                    if *height < Height(NNDataMain::KOMODO_NOTARIES_HARDCODED_MAX_HT) {
                        // must be the old code for hardcoded notaries but it is not implemented and we cover it with checkpoints
                        return Err(NotaryDataError::CheckpointValidate(*height));
                    }
                    return Err(NotaryDataError::NoSeasonForHeight);
                },
            }
        }
    ...
    /// just a wrapper
    fn get_ac_season(&self, timestamp: &DateTime32) -> Result<& NotarySeasonPubkeys, NotaryDataError> {
        match self.nn_season.get_ac_season_internal(timestamp) {
            Some(r) => Ok(r),
            None => Err(NotaryDataError::NoSeasonForTimestamp),
        }
    }

}

/// Notarisation constants: HF activation timestamps and heights, notary pubkeys for testnet
struct NNDataTest<'a> {
    nn_season: NNSeason<'a>,
}
...
impl<'a> NNDataTest<'a> {
    ...
    const NUM_KMD_NOTARIES: usize = 13;
    const NUM_KMD_SEASONS: usize = 1;

    //const KOMODO_NOTARISATION_ACTIVE_TESTNET_HT: u32 = 100;

    const NOTARIES_ELECTED_SOURCE: [[(&'static str, &'static str); NNDataTest::NUM_KMD_NOTARIES]; NNDataTest::NUM_KMD_SEASONS] = [
        [
    ...
    /// make object with NN season constants including pubkeys
    #[allow(non_snake_case)]
    pub fn new() -> NNDataTest<'static>
    {            
        let KMD_SEASON_TIMESTAMPS = vec![DateTime32::from(0)];
        let KMD_SEASON_HEIGHTS = vec![Height(0)];

        // convert to vector for convenience
        let elected_source = NNDataTest::NOTARIES_ELECTED_SOURCE.into_iter().
            map(|v| v.into_iter().collect::<Vec<_>>()).
            collect::<Vec<Vec<_>>>();

        NNDataTest {
            nn_season: NNSeason::new(
                NNDataTest::NUM_KMD_NOTARIES, 
                NNDataTest::NUM_KMD_SEASONS, 
                KMD_SEASON_TIMESTAMPS, 
                KMD_SEASON_HEIGHTS, 
                &elected_source
            ),
    ...
    /// get the kmd season based on height (used on the KMD chain)
    /// a wrapper for testnet
    pub fn get_kmd_season(&self, _height: &Height) -> Result<&'a NotarySeasonPubkeys, NotaryDataError>
    {
        //if *height >= Height(NNDataTest::KOMODO_NOTARISATION_ACTIVE_TESTNET_HT) {
        return match self.nn_season.get_kmd_season_internal_default() {
            Some(r) => Ok(r),
            None => Err(NotaryDataError::NoSeasonForHeight),
        };
        //}
        //Err(NotaryDataError::NoSeasonForHeight)
    }
    ...
    /// get the asset chain season based on timestamp (used on asset chains)
    /// just a wrapper for testnet
    fn get_ac_season(&self, timestamp: &DateTime32) -> Result<&'a NotarySeasonPubkeys, NotaryDataError> {
        match self.nn_season.get_ac_season_internal(timestamp) {
            Some(r) => Ok(r),
            None => Err(NotaryDataError::NoSeasonForTimestamp),
        }
...
/// functions to get season data based on height or timestamp
struct NNSeason<'a> {
...
impl<'a> NNSeason<'a> {
    ...
    #[allow(non_snake_case)]
    pub fn new(num_kmd_notaries: usize, num_kmd_seasons: usize, 
        kmd_season_timestamps: Vec<DateTime32>, kmd_season_heights: Vec<Height>, 
        notaries_elected_source: &Vec<Vec<(&'static str, &'static str)>>) -> NNSeason<'static>
    { 
        assert!(
            kmd_season_timestamps.len() == kmd_season_heights.len() && 
            kmd_season_heights.len() == notaries_elected_source.len() && 
            notaries_elected_source.len() == num_kmd_seasons, "invalid season data lengths");

        NNSeason {
            kmd_season_timestamps, kmd_season_heights, notaries_elected: Self::init_nn_pubkeys(notaries_elected_source, num_kmd_notaries)
        }
    }

    /// Convert const string pubkeys to PublicKey
    fn init_nn_pubkeys(notaries_elected_source: &Vec<Vec<(&'static str, &'static str)>>, num_kmd_notaries: usize) -> Vec<NotarySeasonPubkeys<'static>>
    {
        let nn = notaries_elected_source.into_iter()
            .map(
                |r| { 
                    r.into_iter()
                        .map( |t| { (t.0, PublicKey::from_slice(hex::decode(t.1).expect("valid hex string").as_ref()).expect("valid public key")) } ) // convert string to PublicKeys
                        .collect::<Vec<_>>() 
                })
                .map(|e| { assert!(e.len() == num_kmd_notaries, "each season must have {} pubkeys", num_kmd_notaries); e }) // check length is 64 for all seasons
                .collect::<Vec<Vec<_>>>(); 
    ...
    /// get the kmd season 
    fn get_kmd_season_internal_default(&self) -> Option<&'a NotarySeasonPubkeys>
    {
        if self.notaries_elected.len() == 1  {
            Some(&self.notaries_elected[0])
        } else {
            None 
        }
    }
    ...
    /// get the asset chain season based on timestamp (used on asset chains)
    fn get_ac_season_internal(&self, timestamp: &DateTime32) -> Option<&'a NotarySeasonPubkeys>
    {
...
/// generic to get notary id for pubkey or address for the height or timestamp
pub trait GetNotaryId<T, D> {
    /// get notary id for dest (pk or addr)
    fn komodo_get_notary_id(network: Network, time_or_height: &T, dest: &D) -> Result<Option<NotaryId>, NotaryDataError>;

    /// check if notary id is actual for dest
    fn komodo_is_notary_dest(network: Network, time_or_height: &T, dest: &D) -> bool {
        if let Ok(r) = Self::komodo_get_notary_id(network, time_or_height, dest) {
            if r.is_some() {
                return true;
            }
        }
...
/// wrapper api to access notary season data
pub struct NN;

impl NN {

    /// get notary pubkeys for the season for this height
    pub fn komodo_get_kmd_season(network: Network, height: &Height) -> Result<&'static NotarySeasonPubkeys<'static>, NotaryDataError>
    {
        match network {
            Network::Mainnet => {
                return NNDATA_MAIN.get_kmd_season(height); // panics if the season invalid
            },
            Network::Testnet => {
                return NNDATA_TEST.get_kmd_season(height); // panics if the season invalid
            },
        }
    }

    /// get notary pubkeys for the season for this timestamp
    pub fn komodo_get_ac_season(network: Network, timestamp: &DateTime32) -> Result<&'static NotarySeasonPubkeys<'static>, NotaryDataError>
    {
        match network {
            Network::Mainnet => {
                return NNDATA_MAIN.get_ac_season(timestamp); // panics if the season invalid
            },
            Network::Testnet => {
                return NNDATA_TEST.get_ac_season(timestamp); // panics if the season invalid
            },
        }
    }
...
impl GetNotaryId<Height, PublicKey> for NN {
    /// check if notary id is present in the season for this height
    fn komodo_get_notary_id(network: Network, height: &Height, pk: &PublicKey) -> Result<Option<NotaryId>, NotaryDataError>   {
        let season = NN::komodo_get_kmd_season(network, height)?; 
...
impl GetNotaryId<DateTime32, PublicKey> for NN {
    /// check if notary id is present in the season for this height
    fn komodo_get_notary_id(network: Network, timestamp: &DateTime32, pk: &PublicKey) -> Result<Option<NotaryId>, NotaryDataError>  {
        let season = NN::komodo_get_ac_season(network, timestamp)?; 
        if let Some(nid) = season.iter().position(|&t| { t.1 == *pk }) {
            Ok(Some(nid as NotaryId))
        } else {
            Ok(None)
...
impl GetNotaryId<Height, TransparentAddress> for NN {
    /// check if notary id is present in the season for this height
    fn komodo_get_notary_id(network: Network, height: &Height, taddr: &TransparentAddress) -> Result<Option<NotaryId>, NotaryDataError>   {
        let season = NN::komodo_get_kmd_season(network, height)?;         
        if let Some(nid) = season.iter().position(|&t| { 
            let pk_bin = t.1.serialize();
            TransparentAddress::PublicKey(*ripemd::Ripemd160::digest(Sha256::digest(&pk_bin)).as_ref()) == *taddr 
        }) {
            Ok(Some(nid as NotaryId))
        } else {
            Ok(None)
...
impl GetNotaryId<DateTime32, TransparentAddress> for NN {
    /// check if notary id is present in the season for this height
    fn komodo_get_notary_id(network: Network, timestamp: &DateTime32, taddr: &TransparentAddress) -> Result<Option<NotaryId>, NotaryDataError>   {
        let season = NN::komodo_get_ac_season(network, timestamp)?;         
        if let Some(nid) = season.iter().position(|&t| { 
            let pk_bin = t.1.serialize();
            TransparentAddress::PublicKey(*ripemd::Ripemd160::digest(Sha256::digest(&pk_bin)).as_ref()) == *taddr 
...
impl NN {
    ...
    /// Check if a tx output signed by a due KMD notary node for a height
    pub fn komodo_get_notary_id_for_spent_output(network: Network, height: &Height, output: &Output) -> Option<NotaryId> {
        let script = zcash_primitives::legacy::Script::from(&output.lock_script);

        if let Some(addr) = script.address() {
            return match <NN as GetNotaryId<Height, TransparentAddress>>::komodo_get_notary_id(network, height, &addr) {
                Ok(id) => id,
                Err(_) => None,
            };
        }
    ...
    /// wrapper on the generic returning notaryid for height, to avoid adding PublicKey import, if you do not want to add such a dependency 
    pub fn komodo_get_notary_id(network: Network, height: &Height, pk: &PublicKey) -> Result<Option<NotaryId>, NotaryDataError> {
        <NN as GetNotaryId<Height, PublicKey>>::komodo_get_notary_id(network, height, pk)
    }
    /// wrapper on the generic checking if pubkey is a notary for height, to avoid adding PublicKey import, if you do not want to add such a dependency 
    pub fn komodo_is_notary_pubkey(network: Network, height: &Height, pk: &PublicKey) -> bool {
        <NN as GetNotaryId<Height, PublicKey>>::komodo_is_notary_dest(network, height, pk)
    }

    /// wrapper on the generic returning notaryid for timestamp, to avoid adding PublicKey import, if you do not want to add such a dependency 
    pub fn komodo_get_ac_notary_id(network: Network, datetime: &DateTime32, pk: &PublicKey) -> Result<Option<NotaryId>, NotaryDataError> {
        <NN as GetNotaryId<DateTime32, PublicKey>>::komodo_get_notary_id(network, datetime, pk)
    }
    /// wrapper on the generic checking if pubkey is a notary for timestamp, to avoid adding PublicKey import, if you do not want to add such a dependency 
    pub fn komodo_is_ac_notary_pubkey(network: Network, datetime: &DateTime32, pk: &PublicKey) -> bool {
        <NN as GetNotaryId<DateTime32, PublicKey>>::komodo_is_notary_dest(network, datetime, pk)
    }

    /// returns if dpow notarisation protocol active (since ht=34_000)
    pub fn komodo_notarisation_active(
        network: Network,
        height: &Height,
    ) -> bool {
        match (network, height) {
            (Network::Mainnet, height) => {
                *height > Height(NNDataMain::KOMODO_NOTARISATION_ACTIVE_HT)                
            },
            (Network::Testnet, _) => {
                *height > Height(66)             // for decker testnet with test notarisation support, we need at least 66 blocks  
            },
        }
    }

    /// checking if looking down 66 blocks is active (nothing actually is down until following rules are active)
    pub fn komodo_notaries_new_depth_rule_active(
        network: Network,
        height: &Height,
    ) -> bool {
        match (network, height) {
    ...
    /// checking if check for duplicates in 66 blocks is active (since ht=792000 in mainnet)
    pub fn komodo_notaries_check_for_dups_rule_active(
        network: Network,
        height: &Height,
    ) -> bool {
        match (network, height) {
            (Network::Mainnet, height) => {
                *height > Height(792000)                
            },
            (Network::Testnet, _) => {
                false // *height > Height(0)              
            },
        }
    }

    /// checking if check for duplicates in 66 blocks is active (since ht=807000 in mainnet)
    pub fn komodo_notaries_check_blocktime_active(
        network: Network,
        height: &Height,
    ) -> bool {
        match (network, height) {
            (Network::Mainnet, height) => {
                *height > Height(807000)                
            },
            (Network::Testnet, _) => {
                false   // *height > Height(0)              
            },
    ...
    pub fn komodo_is_gap_after_second_block_allowed(
        ...
        match (network, *height) {
            (Network::Mainnet, height) => {
                let height_s6 = NNDATA_MAIN.nS6HardforkHeight;
    ...
    pub fn komodo_s3_5_december_hardfork_active(
        ...
        match (network, height) {
            (Network::Mainnet, height) => {
                return *height > NNDATA_MAIN.nDecemberHardforkHeight;
            },
            (Network::Testnet, _) => {
                false
            },
    ...
    /// returns if new hardcode notaries still active
    pub fn komodo_hardcoded_notaries_ended(
        network: Network,
        height: &Height,
    ) -> bool {
        match (network, height) {
...
mod tests {
    ...
    fn nn_spk_height_too_early() {
        ...
        assert!(<NN as GetNotaryId<Height, PublicKey>>::komodo_get_notary_id(Network::Mainnet, &Height(64105), &pk).is_err(), "notary pubkey check broken");
    }
