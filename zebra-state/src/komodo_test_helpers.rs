...
pub fn komodo_load_testnet_node_1() -> Vec<Arc<Block>> {
    
    let genesis_bin = Vec::from_hex(include_str!("./service/check/tests/testnet-genesis.hex").trim()).expect("invalid genesis hex");
    let genesis = genesis_bin.zcash_deserialize_into::<Arc<Block>>()
        .expect("block should deserialize");
    let mut blocks: Vec<Arc<Block>> = vec![genesis.clone()];

    let blocks_node1_hex = include_str!("./service/check/tests/testnet-node-1.hex").split("\n").collect::<Vec<_>>();
